...
echo "2) 检测并配置了IPv4/IPv6环境，确保网络访问正常。"
echo "3) 设置了SSH端口，增强了远程登录安全性。"
echo "4) 启用了防火墙并配置了常用端口，特别是 SSH 服务端口。"
echo "5) 启用了 Fail2Ban 防护，增强了系统安全性。"
echo "6) 根据您的选择，已调整系统时区设置。"
echo "7) 根据您的选择，已调整或配置了 SWAP 大小。"
echo "8) 根据您的选择，已设置BBR。"
echo "9) 清理了系统垃圾文件和临时文件。"
echo "10) 根据您的选择，已清理了不需要的系统日志文件。"
