...
if command -v apt &> /dev/null; then
    ...
    echo -e "${RED}错误：未检测到 apt 或 yum，无法更新系统${NC}"
    exit 1
fi
...
if ! command -v sudo &> /dev/null; then
    echo -e "${YELLOW}警告：未检测到 sudo，正在安装...${NC}"
...
if ! command -v wget &> /dev/null; then
    echo -e "${YELLOW}警告：未检测到 wget，正在安装...${NC}"
...
if ! command -v curl &> /dev/null; then
    echo -e "${YELLOW}警告：未检测到 curl，正在安装...${NC}"
...
if ! command -v fail2ban-client &> /dev/null; then
    echo -e "${YELLOW}警告：未检测到 fail2ban，正在安装...${NC}"
...
if ! command -v ufw &> /dev/null; then
    echo -e "${YELLOW}警告：未检测到 ufw，正在安装...${NC}"
...
check_and_set_network_priority() {
    ...
    if [ -z "$ipv4_address" ]; then
        echo -e "${YELLOW}提示：本机无IPv4地址。${NC}"
    else
        echo -e "本机IPv4地址 : ${GREEN}$ipv4_address${NC}"
    fi

    if [ -z "$ipv6_address" ]; then
        echo -e "${YELLOW}提示：本机无IPv6地址。${NC}"
    else
    ...
    if sysctl net.ipv6.conf.all.prefer_ipv4 &>/dev/null; then
        ...
        if [ "$ipv4_preference" == "1" ]; then
            ...
            if [ "$ipv4_valid" == false ] && [ "$ipv6_valid" == true ]; then
                echo -e "${YELLOW}提示：检测到本机为IPv6 only网络环境。${NC}"
        ...
        echo -e "当前系统的优先级设置 : ${GREEN}$current_preference${NC}"
    else
        if [ "$ipv4_valid" == false ] && [ "$ipv6_valid" == true ]; then
            echo -e "${YELLOW}提示：检测到本机为IPv6 only网络环境。${NC}"
        fi
    ...
    if [ -n "$ipv4_address" ] && [ -n "$ipv6_address" ] && [ "$ipv6_valid" == true ] && [ "$ipv4_valid" == true ]; then
        ...
        if [ "$ipv4_valid" == false ] && [ "$ipv6_valid" == true ]; then
            echo -e "${YELLOW}提示：本机为IPv6 only模式，IPv4不可用。${NC}"
        elif [ "$ipv6_valid" == false ] && [ "$ipv4_valid" == true ]; then
            echo -e "${YELLOW}提示：本机为IPv4 only模式，IPv6不可用。${NC}"
        else
            echo -e "${RED}错误：本机既不可用IPv4，也不可用IPv6，请检查网络配置。${NC}"
        fi
    fi
}
...
configure_ssh_port() {
    ...
    if [[ "$modify_choice" == "y" || "$modify_choice" == "Y" ]]; then
        ...
        if [ -f "$ssh_config_file" ]; then
            ...
            echo -e "${YELLOW}已备份SSH配置文件到 ${ssh_config_file}.bak${NC}"
        ...
        if [ "$current_port_in_ssh_config" -eq "$new_port" ]; then
            echo -e "${GREEN}SSH端口修改成功，新端口为 ${new_port}${NC}"
        else
...
# 检查防火墙并开放新端口
check_firewall() {
    echo -e "${BLUE}正在检查防火墙状态并开放新端口...${NC}"
    echo

    if command -v ufw >/dev/null 2>&1; then
        # ufw防火墙启用检查
        if ! sudo ufw status | grep -q "Status: active"; then
            echo -e "${YELLOW}提示：防火墙未启用，且新端口未被防火墙阻拦。${NC}"
        else
            # 检查新端口是否已在防火墙规则中放行
...
# 安装并启动SSH服务
install_ssh() {
    echo -e "${BLUE}正在安装并启动SSH服务...${NC}"
    echo

    if [[ "$SYSTEM_NAME" == "Ubuntu" || "$SYSTEM_NAME" == "Debian" ]]; then
        # Ubuntu/Debian 系统
        if ! systemctl is-active --quiet ssh; then
            echo -e "${YELLOW}提示：SSH 服务未安装或未启动，正在安装 SSH 服务...${NC}"
        ...
        echo -e "${RED}错误：无法识别的操作系统：$SYSTEM_NAME，无法处理 SSH 服务。${NC}"
        return  # 跳过当前功能块，继续执行后续部分
    fi
}
...
# 检查是否安装了ufw
if ! command -v ufw &>/dev/null; then
    echo -e "${YELLOW}警告：未检测到 ufw，正在安装 ufw...${NC}"
    if command -v apt &>/dev/null; then
        # Ubuntu 或 Debian 系统
        apt update
        apt install -y ufw
...
if ! sudo ufw status &>/dev/null; then
    echo -e "${YELLOW}提示：正在启用 ufw 防火墙...${NC}"
    sudo ufw enable
fi
...
if [ -n "$new_port" ]; then
    sudo ufw allow $new_port/tcp
    echo -e "新端口 ${GREEN}$new_port${NC} 已开放。"
else
    echo -e "${YELLOW}警告：未检测到新的SSH端口，跳过开放新端口。${NC}"
fi
...
if [ -n "$current_port" ] && [ "$current_port" != "$new_port" ]; then
    ...
    echo -e "旧端口 ${GREEN}$current_port${NC} 已关闭。"
else
    echo -e "${YELLOW}提示：当前端口与新端口相同或未检测到旧端口，跳过关闭旧端口。${NC}"
fi
...
if [ "$ssh_ports" != "22" ] && [ -n "$selected_port" ]; then
    echo -e "${BLUE}正在开放所选的 SSH 端口 ${GREEN}$selected_port${NC}...${NC}"
    ...
    case "$swap_choice" in
        1)
            ...
            if [ "${#swap_files[@]}" -gt 0 ]; then
                ...
                # 禁用 SWAP 文件
                swapoff "$selected_swap_file" || { echo -e "${RED}错误：无法禁用 SWAP 文件 $selected_swap_file。${NC}"; return; }

                # 删除 SWAP 文件
                rm -f "$selected_swap_file" || { echo -e "${RED}错误：无法删除 SWAP 文件 $selected_swap_file。${NC}"; return; }

                # 创建新的 SWAP 文件
                fallocate -l "${new_swap_size}M" "$selected_swap_file" 2>/dev/null || {
                    echo -e "${BLUE}fallocate 不可用，使用 dd 创建 SWAP 文件...${NC}"
                    dd if=/dev/zero bs=1M count="$new_swap_size" of="$selected_swap_file" status=progress || { echo -e "${RED}错误：无法创建 SWAP 文件 $selected_swap_file。${NC}"; return; }
                }

                chmod 600 "$selected_swap_file"
                mkswap "$selected_swap_file" || { echo -e "${RED}错误：无法格式化 SWAP 文件 $selected_swap_file。${NC}"; return; }
                swapon "$selected_swap_file" || { echo -e "${RED}错误：无法启用 SWAP 文件 $selected_swap_file。${NC}"; return; }

                # 备份 /etc/fstab
                sudo cp /etc/fstab /etc/fstab.bak
                echo -e "${YELLOW}已备份 /etc/fstab 到 /etc/fstab.bak${NC}"
                ...
                # 禁用 SWAP 分区
                swapoff "$selected_swap_partition" || { echo -e "${RED}错误：无法禁用 SWAP 分区 $selected_swap_partition。${NC}"; return; }
                ...
                parted /dev/"$disk" --script resizepart "$partition_number" "${new_swap_size}MB" || { echo -e "${RED}错误：无法调整分区大小 $selected_swap_partition。请手动检查分区状态。${NC}"; return; }

                # 重新格式化为 SWAP 分区
                echo -e "${BLUE}正在格式化分区 $selected_swap_partition 为 SWAP...${NC}"
                mkswap "$selected_swap_partition" || { echo -e "${RED}错误：无法格式化 SWAP 分区 $selected_swap_partition。${NC}"; return; }

                # 启用 SWAP 分区
                swapon "$selected_swap_partition" || { echo -e "${RED}错误：无法启用 SWAP 分区 $selected_swap_partition。${NC}"; return; }
                ...
                echo -e "${YELLOW}提示：未检测到 SWAP 文件或 SWAP 分区。${NC}"
                echo -e "${BLUE}是否要创建一个新的 SWAP 文件？${NC}"
                read -p "请输入 (y/n): " create_swap
                if [[ "$create_swap" == "y" || "$create_swap" == "Y" ]]; then
                    echo -e "${GREEN}开始创建新的 SWAP 文件...${NC}"
                    sudo fallocate -l "${new_swap_size}M" /swapfile 2>/dev/null || {
                        echo -e "${BLUE}fallocate 不可用，使用 dd 创建 SWAP 文件...${NC}"
                        sudo dd if=/dev/zero bs=1M count="$new_swap_size" of=/swapfile status=progress || { echo -e "${RED}错误：无法创建 SWAP 文件 /swapfile。${NC}"; return; }
                    }
                    sudo chmod 600 /swapfile
                    sudo mkswap /swapfile || { echo -e "${RED}错误：无法格式化 SWAP 文件 /swapfile。${NC}"; return; }
                    sudo swapon /swapfile || { echo -e "${RED}错误：无法启用 SWAP 文件 /swapfile。${NC}"; return; }
                    ...
                    echo -e "${GREEN}已成功创建并启用新的 SWAP 文件 /swapfile，大小为 ${new_swap_size} MB。${NC}"
                else
                    echo -e "${YELLOW}未创建新的 SWAP 文件。${NC}"
                fi
            fi
...
if [ "$bbr_modified" = true ]; then
    echo -e "${YELLOW}提示：刚才修改了BBR设置，需要重启后才能生效。${NC}"
